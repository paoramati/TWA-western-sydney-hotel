

<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <title>Western Sydney Hotel: Homepage</title>
		<link rel="stylesheet" type="text/css" href="bmp_style.css">
    </head>
    <body>
		<div id="head">
			<h1>Western Sydney Hotel</h1>
		</div>
	
		<div class="nav">
		<ul class=\"nav\">
		<li class=\"nav\"><a href=\"index.php\">Home</a></li>
		<li class=\"nav\"><a href=\"register.php\">Customer Registration</a></li>
		<li class=\"nav\"><a href=\"customerlogin.php\">Customer Login</a></li>
		<li class=\"nav\"><a href=\"adminlogin.php\">Administrator Login</a></li>
		</ul>
			
		</div>
			
			
		<div id="section">
			<h2>Welcome to the Western Sydney Hotel!</h2>
			
			<p>The Western Sydney Hotel is New South Wale's premier hotel.<br>
			<img id="lobby" src="lobby3.jpg" alt="WSH Lobby" width="350" height="300">
			Founded in 2016, the WSH is your 5-star link to the city and the beautiful Blue Mountains.</p>
			
			
		</div>
		
		<div class="instructions">
			
			<p><strong>Help</strong></p>
			<p>Home: This page</p>
			<p>Customer Registration: Register with WSH to view and book rooms</p>
			<p>Customer Login: If you are registered, login to view rooms and making bookings</p>
			<p>Administrator Login: Staff login page</p>
		</div>
		
		
		
		
		
			
			
			
			
			

    </body>
</html>
